...
// TODO: Make a global `app` object (à la Flask), and hang `redis` off it.
Game.redis = db.redis();
 ...
 */
Game.all = function () {
  return this.redis.hvals('game').then(function (values) {
...
Game.create = function (data) {
  return new Promise(function (resolve, reject) {
    return this.redis.hexists('game', data.slug).then(function (val) {
      if (val === 1) {
        // Bail if slug is already taken.
        return reject("Duplicate entry '" + data.slug + "' for key 'slug'");
      }

      return this.redis.hset('game', data.slug, JSON.stringify(data))
      .then(function () {
        resolve();
      });
    }.bind(this));
...
Game.get = function (data) {
  return new Promise(function (resolve, reject) {
    ...
    return this.redis.hget('game', data.slug).then(function (gameData) {
      if (gameData === null) {
...
Game.delete = function (game) {
  return new Promise(function (resolve, reject) {
    ...
    return this.redis.hdel('game', game.data.slug)
    .then(resolve);
  }.bind(this));
};
...
Game.update = function (game, payload) {
  return new Promise(function (resolve, reject) {
    ...
    if (newGameData.slug !== gameData.slug) {
      return this.redis.hdel('game', gameData.slug).then(function () {
        resolve(newGameData);
      });
    } else {
      resolve(newGameData);
