...
user.prototype.authenticate = function(email) {
    var userData = this.dataChannel.hget('users', email);
    if (!userData) {
        userData = {
            username: email.split('@')[0],
            email: email
        };
        this.dataChannel.hset('users', email, JSON.stringify(userData))
    } else {
...
user.prototype.updateLeaderboard = function(board, value) {
    ...
    if (!leaderboard.isLeaderboard(this.dataChannel, currentlyPlaying, board)) {
        return;
    }
