...
user.prototype.authenticate = function(email, callback) {
    ...
    getUserFromEmail(this.dataChannel, email, function(err, userData) {
        ...
        if (!userData) {
            ...
            self.dataChannel.hset('users', userData.id, JSON.stringify(userData));
            self.dataChannel.hset('usersByEmail', email, userData.id);
            finishAuth(userData);
        } else {
...
function getUserFromEmail(client, email, callback) {
    client.hget('usersByEmail', email, function(err, resp) {
        if (err) {
            ...
            // `resp` is the user ID.
            getUserFromID(client, resp, callback);
        }
    });
}
