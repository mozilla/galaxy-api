...
function getGameList(client, ids, filters, callback) {
    ...
    if (!ids.length) {
        ...
        client.hgetall('games', handleResults);
    } else {
    ...
    function handleResults(err, resp) {
        ...
        var parsed = resp.map(function(result) {
            if (!full) return;
            try {
        ...
        if (!filterKeys.length) {
            ...
            callback(parsed.filter(function(game) {
                return _.every(filterKeys, function(key) {
                    var pred = filters[key];
                    var val = game[key];
                    switch (typeof pred) {
                        case 'function':
                            return pred(val);
...
function getPublicGameObjList(client, ids, filters, callback) {
    getGameList(client, ids, filters, function(result) {
